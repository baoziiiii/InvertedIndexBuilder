...
int read_intermediate(char** term_id_map, term_entry* buffer[], int max, int file_id){
    char filename[20];
    snprintf(filename, sizeof(filename), "%s-%d", "tmp/intermediate", file_id);
...
int sort(int num_intermediate_files){
    ...
    for(int i = 0; i < num_intermediate_files; i++){
        ...
        int readed = read_intermediate(term_id_map, sort_buffer, INTERMEDIATE_SIZE, i);
        qsort(sort_buffer, readed, sizeof(term_entry*), Comparator);
        FILE* f = fopen(filename, "w");
        if(f == NULL){
            printf("Error: No file named: %s", filename);
            exit(EXIT_FAILURE);
        }
